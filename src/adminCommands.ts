...
import {
  getCategoryPrompt,
  upsertCategoryPrompt,
  listCategoryPrompts,
  deleteCategoryPrompt,
  type CategoryPromptConfig
} from "./memory/memoryDb.js";
const getModelStatsCommand = async (_args: string[]) =>
  "Model stats are not available in Ollama mode.";
  ...
  try {
    ...
    switch (mainCommand) {
      ...
      case 'category':
        ...
        if (subCommand === 'list') {
          const categories = await listCategoryPrompts();
          if (categories.length === 0) {
            response = '📂 No category prompts configured.';
          } else {
            response = '📂 **Category Prompts**:\n\n' +
              categories.map(c =>
                `**${c.category_name || c.category_id}** (${c.enabled ? '✅ Enabled' : '❌ Disabled'})\n` +
                `   ID: \`${c.category_id}\`\n` +
                `   Prompt: ${c.prompt_modifications.substring(0, 100)}${c.prompt_modifications.length > 100 ? '...' : ''}`
              ).join('\n\n');
          }
          ...
          const config = await getCategoryPrompt(categoryId);
          if (!config) {
          ...
          await upsertCategoryPrompt(config);
          ...
          const categoryId = args[1];
          await deleteCategoryPrompt(categoryId);
          ...
          const categoryId = args[1];
          const existing = await getCategoryPrompt(categoryId);
          if (!existing) {
            response = `❌ **Category not found**: \`${categoryId}\`\nCreate it first with \`!category set\``;
            break;
          }
          await upsertCategoryPrompt({ ...existing, enabled: true });
          ...
          await upsertCategoryPrompt({ ...existing, enabled: false });
