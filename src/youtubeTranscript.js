...
exports.preprocessYouTubeLinks = preprocessYouTubeLinks;
exports.handleChunkRequest = handleChunkRequest;
exports.cleanupTranscriptCache = cleanupTranscriptCache;
exports.getMostRecentVideo = getMostRecentVideo;
...
async function getYouTubeClient() {
    if (!youtubeClient) {
        ...
        youtubeClient = await youtubei_js_1.Innertube.create();
 ...
 */
function createChunksWithTimestamps(segments, chunkSize) {
    const chunks = [];
    let currentChunk = '';
 ...
 */
function estimateDuration(length) {
 ...
 */
async function processTranscript(videoId, fullText, language, title, segments) {
    const metadata = {
        videoId,
        title,
        language,
 ...
 */
async function preprocessYouTubeLinks(content, sendTyping) {
    ...
    for (const match of matches) {
        ...
        const transcriptText = await processTranscript(videoId, transcriptData.text, transcriptData.language, transcriptData.title, transcriptData.segments);
 ...
 */
function findChunkByTimestamp(chunks, targetMs) {
 ...
 */
function handleChunkRequest(content) {
    ...
    if (infoMatch) {
        ...
        const chunkList = cached.chunks.map(chunk => `  📍 Chunk ${chunk.index}: ${chunk.startTime} - ${chunk.endTime}`).join('\n');
        return `
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 */
function cleanupTranscriptCache() {
    const now = Date.now();
    let removed = 0;
