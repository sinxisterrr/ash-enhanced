...
async function sendChunkedToChannel(channel, content) {
    if (!channel || !content)
        return;
...
function buildSyntheticMessage(content, userId) {
    return {
        content,
...
async function runSystemPrompt(content) {
    ...
    const reply = await (0, handleMessage_js_1.handleMessage)(synthetic, {
...
async function sendMessage(message, _messageType, conversationContext = null, customContent = null) {
    await (0, index_js_1.initAshSystems)();
...
async function sendTaskMessage(task, channel, _discordClient) {
...
async function getSpotifyNowPlaying() {
    ...
    try {
        ...
        if (!accessToken)
            return null;
        const nowPlayingResp = await fetch("https://api.spotify.com/v1/me/player/currently-playing", {
            headers: {
