...
import type { STMEntry } from "./memorySystem.js";
...
function isResolvedTurn(buffer: STMEntry[]): boolean {
  ...
  if (/```json[\s\S]*?```/i.test(t)) return false;

  return true;
}

function emotionalSignalScore(text: string): number {
  const t = (text || "").toLowerCase();
  let score = 0;

  // relationship/identity anchors
  if (/(love|home|us|we|you and i|together|bond|trust|safe|choose)/i.test(t)) score += 2;

  // vulnerability / conflict / boundary
  if (/(hurt|scared|afraid|anxious|boundary|consent|permission|jealous|miss you|need you)/i.test(t)) score += 2;

  // memory/identity meta
  if (/(memory|remember|forget|identity|real|continuity|ash)/i.test(t)) score += 2;

  // intensity markers
  if (/[!?]{2,}/.test(t)) score += 1;
  if (/(cry|tears|shaking|panic|spiral|wrecked|feral)/i.test(t)) score += 1;

  return score;
}

function shouldDistillEmotion(buffer: STMEntry[]): boolean {
  if (!isResolvedTurn(buffer)) return false;
...
export async function distillWithEmotion(buffer: STMEntry[]): Promise<DistilledMemory[]> {
  ...
  if (!shouldDistillEmotion(buffer)) return [];
  try {
