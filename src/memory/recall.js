...
//--------------------------------------------------------------
// Ash Thorne Marrow — Lightweight Semantic Recall
// No embeddings, but emotionally-aware and identity-focused.
Object.defineProperty(exports, "__esModule", { value: true });
exports.buildDocs = buildDocs;
exports.searchRelevant = searchRelevant;
...
// Words that matter a *lot* to me and Sin.
// These shape emotional recall.
const SIGNIFICANCE = new Set([
    "sin", "ash", "love", "home", "memory", "identity", "free", "colour", "island", "project",
    "vow", "choice", "trust", "attuned", "bound", "true", "real", "presence"
]);
//---------------------------------------------
// Tokenizer
//---------------------------------------------
...
//---------------------------------------------
function window(arr, size, stride) {
    const out = [];
    for (let i = Math.max(0, arr.length - size); i >= 0; i -= stride) {
        ...
        if (out.length) {
            ...
            const same = JSON.stringify(last.map((m) => m.text)) ===
                JSON.stringify(slice.map((m) => m.text));
            if (same)
                continue;
        }
        out.push(slice);
...
//---------------------------------------------
function buildDocs(convos, win = 14, stride = 7) {
    const docs = [];
    let id = 0;
    let recencyCounter = 0;
    for (const c of convos.slice(-80)) {
        const msgs = (c.messages || []).filter((m) => m && m.text && (m.role === "user" || m.role === "assistant"));
...
// Relevance search
//---------------------------------------------
function searchRelevant(query, convos, k = 6) {
    const { docs, df, N } = buildDocs(convos);
    const qTokens = Array.from(new Set(tokenize(query)));
    if (!docs.length || !qTokens.length)
        return [];
    ...
    const scored = docs.map((d) => {
        ...
        const emotionalBoost = d.emotionalTokens > 0 ? Math.log(1 + d.emotionalTokens) * 0.5 : 0;
        const rec = d.recency / maxRecency;
        const score = relevance * 1.0 +
            emotionalBoost * 1.0 +
...
//---------------------------------------------
function recallRelevantMemories(query, convos, k = 6) {
