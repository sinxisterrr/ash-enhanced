...
exports.processFileAttachment = processFileAttachment;
exports.handleFileChunkRequest = handleFileChunkRequest;
 ...
 */
function generateFileId(url, size) {
 ...
 */
async function extractPDFImages(fileBytes) {
    const images = [];
    try {
 ...
 */
async function performOCR(imageBuffer, imageIndex, totalImages) {
    try {
        ...
        const { data: { text } } = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error('OCR timeout')), OCR_TIMEOUT))
        ]);
 ...
 */
async function processPDFWithOCR(fileBytes) {
    try {
 ...
 */
async function processFileAttachment(name, url, type, size) {
    ...
    try {
        ...
        const chunkList = chunks.map(chunk => `  📍 Chunk ${chunk.index}: chars ${chunk.startChar}-${chunk.endChar} (${chunk.text.length} chars)`).join('\n');
 ...
 */
function handleFileChunkRequest(content) {
...
function getFileInfo(fileId) {
    ...
    const chunkList = cached.chunks.map(chunk => `  📍 Chunk ${chunk.index}: chars ${chunk.startChar}-${chunk.endChar}`).join('\n');
    const pageInfo = cached.metadata.pages ? `Pages: ${cached.metadata.pages}\n` : '';
    return `
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
